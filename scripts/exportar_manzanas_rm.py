...
Salidas:
  src/data/manzanas_rm.geojson   — 48k manzanas con atributos censales
  src/data/comunas_rm.geojson    — 52 comunas (disueltas desde manzanas por CUT)
...
OUT_COMUNAS = ROOT / "public" / "data" / "comunas_rm.geojson"
...
def main():
    ...
    manzanas_features = []
    comunas_geoms = defaultdict(list)
    comunas_meta = {}
    comunas_pobl = defaultdict(int)

    print("[2/4] Filtrando RM y simplificando ...")
    for i, sr in enumerate(reader.iterShapeRecords(fields=CAMPOS), start=1):
        ...
        # Acumular para disolver comunas (en mercator, más barato)
        comunas_geoms[cut].append(geom_sh)
        comunas_meta[cut] = {
            "comuna": sr.record["COMUNA"].strip(),
            "provincia": sr.record["PROVINCIA"].strip(),
        }

        pers = to_int(sr.record["TOTAL_PERS"])
        comunas_pobl[cut] += pers
    ...
    print("[3/4] Disolviendo comunas ...")
    comunas_features = []
    for cut, geoms in comunas_geoms.items():
        disuelta = unary_union(geoms)
        disuelta = disuelta.simplify(25.0, preserve_topology=True)  # borde de comuna: tolerancia mayor
        geom_4326 = reproject_geom(disuelta)
        geom_json = round_coords(mapping(geom_4326), ndigits=5)
        meta = comunas_meta[cut]
        comunas_features.append({
            ...
            "properties": {
                ...
                "pob": comunas_pobl[cut],
                "n_manzanas": len(geoms),
            },
        })
    print(f"      comunas: {len(comunas_features)}")

    print("[4/4] Escribiendo GeoJSONs ...")
    ...
    with open(OUT_COMUNAS, "w", encoding="utf-8") as f:
        json.dump({"type": "FeatureCollection", "features": comunas_features}, f,
                  ensure_ascii=False, separators=(",", ":"))

    size_m = OUT_MANZANAS.stat().st_size / 1_048_576
    size_c = OUT_COMUNAS.stat().st_size / 1_048_576
    print(f"      {OUT_MANZANAS.name}: {size_m:.1f} MB")
    print(f"      {OUT_COMUNAS.name}:  {size_c:.2f} MB")
    print("[OK] listo")
